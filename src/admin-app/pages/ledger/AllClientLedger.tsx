...
//                                   setShowModal(false);
//                                   const res = await betService. newledger();
...
const AllClientLedger = () => {
  ...
  const fetchLedger = async () => {
    const res: AxiosResponse<{ data: any }> = await betService. newledger();
    const { lenaArray, denaArray } = res.data.data;
    ...
    <div style={{ zoom: 0.8 }}>
      ...
      <div className="ledger-container">
          ...
          <table className="ledger-table">
            <thead>
              <tr>
                ...
                <th>AMOUNT</th>
                <th>SETTLED</th>
                <th className="bg-final text-white">FINAL</th>
                <th>ACTION</th>
              </tr>
            </thead>
        ...
        {/* {showModal && selectedEntry && (
          <div className="modal-overlay">
            <div className="modal-content modal-fit-width p-3">
              <h3>Settle with: {selectedEntry.agent}</h3>
              ...
              <button
                ...
                onClick={async () => {
                  ...
                  if (inputAmount > Math.abs(selectedEntry.final)) {
                    alert("Amount exceed");
                    return;
                  }
                  ...
                  await betService.postsettelement({
                    ChildId: selectedEntry.ChildId,
                    settleamount,
                    remark,
                    type: modalType,
...
export default React.memo(AllClientLedger);